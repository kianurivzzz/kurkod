class QRCustomizer {
	...
	showPanel() {
		...
		const logoRadio = Array.from(this.logoRadios).find(
			radio => radio.value === this.settings.logo
		);
		if (logoRadio) {
	...
	handleLogoChange(radio) {
		...
		// Если выбран дефолтный логотип, сбрасывает кастомный
		if (!isCustom) {
	...
	handleFileUpload(event) {
		...
		if (file && file.type.startsWith('image/')) {
			...
			reader.onload = e => {
				...
				// Автоматически выбирает "custom" при загрузке файла
				const customRadio = Array.from(this.logoRadios).find(
	...
	applyChanges() {
		...
		this.saveSettings();
		// Перегенерирует QR-код с текущими настройками
	...
	resetChanges() {
		...
		const defaultRadio = Array.from(this.logoRadios).find(
			radio => radio.value === 'default'
		);
		if (defaultRadio) {
