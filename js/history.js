class QRHistory {
    ...
    formatDate(timestamp) {
        ...
        if (isToday) {
            return `${translations.today}, ${date.toLocaleTimeString(lang, { hour: '2-digit', minute: '2-digit' })}`;
        } else if (isYesterday) {
            return `${translations.yesterday}, ${date.toLocaleTimeString(lang, { hour: '2-digit', minute: '2-digit' })}`;
        } else {
    ...
    renderHistory() {
        ...
        this.historyList.innerHTML = history.map(item => `
            <a href="#" class="history-item" data-text="${item.text}">
                ...
                <button class="history-item-delete" aria-label="${translations.deleteItem}" data-timestamp="${item.timestamp}">
        ...
        this.historyList.querySelectorAll('.history-item').forEach(item => {
            // Click on item to generate QR code
        ...
        this.historyList.querySelectorAll('.history-item-delete').forEach(button => {
